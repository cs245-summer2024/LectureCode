...
public class GraphAdjacencyMatrix<T> {
    /*
        ...
        we would this use map to map each city name to an internal id that would be used
    ...
    private boolean isConnected(int source, int target, Set<Integer> visitedNodes) {
        if(source == target) {
            return true;
        }
        ...
        for(int neighbor = 0; neighbor < getNumVertices(); neighbor ++) {
            if(adjNeighbors[source][neighbor] == 1 && !visitedNodes.contains(neighbor)) {
                pathExists = pathExists || isConnected(neighbor, target, visitedNodes);
                if(pathExists) {
                    return true;
                }
            }
    ...
    private void allPathsHelper(int node, boolean[] visited, List<Integer> curPath, List<String> allPaths) {
        ...
        for(int neighborIdx = 0; neighborIdx < getNumVertices(); neighborIdx ++) {
            if(adjNeighbors[node][neighborIdx] == 1 && !visited[neighborIdx]) {
                allPathsHelper(neighborIdx, visited, curPath, allPaths);
            }
        }
